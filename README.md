...
> 💡 **Совет:** В папке **`scripts/`** лежат готовые файлы для запуска двойным кликом (как `.bat` в Windows):
> - 🟢 `ЗАПУСТИТЬ_DOCKER.command`
> - 🔵 `ЗАПУСТИТЬ_KUBERNETES.command`
> - 🔴 `ОСТАНОВИТЬ_ВСЁ.command`
